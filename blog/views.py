...
from .models import Post, Category, Comment, Tag, Photo, PhotoProfile, UserProfile
...
def profile(request):
    ...
    if request.method == 'POST':
        ...
        if profile_form.is_valid():
            ...
            user.date_of_birth = profile_form.cleaned_data.get('date_of_birth')
            user.country = profile_form.cleaned_data.get('country')
            user.group = profile_form.cleaned_data.get('group')
            use_existing_photo = profile_form.cleaned_data.get('use_existing_photo')

            if use_existing_photo:
                existing_photo = PhotoProfile.objects.all()
                user.avatar = existing_photo.image
            else:
                new_photo = profile_form.cleaned_data.get('new_photo')
